...
function make_simple_chain(input_deint, input_scale, hq)
	...
	local wb_effect = chain:add_effect(WhiteBalanceEffect.new())
	local lgg_effect = chain:add_effect(LiftGammaGainEffect.new())
	...
	return {
		...
		resample_effect = resample_effect,
		resize_effect = resize_effect,
		lgg_effect = lgg_effect
	}
end

function set_lift_gamma_gain(chain_or_input, lgg)
	if (chain_or_input.lgg_effect) then
		chain_or_input.lgg_effect:set_vec3("lift", lgg[1], lgg[1], lgg[1])
		chain_or_input.lgg_effect:set_vec3("gamma", lgg[2], lgg[2], lgg[2])
		chain_or_input.lgg_effect:set_vec3("gain", lgg[3], lgg[3], lgg[3])
	end
